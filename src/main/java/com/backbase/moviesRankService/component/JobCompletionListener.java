...
public class JobCompletionListener extends JobExecutionListenerSupport {
    ...
    public void afterJob(JobExecution jobExecution) {
        if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
          ...
          /*  JobParameters jobParameters = jobExecution.getJobParameters();
            ...
            /*
            List<AcademyAward> results = jdbcTemplate.query("SELECT DISTINCT ip_address, COUNT( ip_address ) as request_made FROM `access_log` WHERE (date_event between '" + startDate + "' AND '" + endDate + "') GROUP BY ip_address", new RowMapper<AcademyAward>() {
                @Override
                public AcademyAward mapRow(ResultSet rs, int row) throws SQLException {
                    ...
                    if (rs.getInt(2) >= getThreshold()) {
                        ...
                        AcademyAward blockedAccess = new AcademyAward();
    ...
    /*
    public void insertBatch(final List<AcademyAward> access) {
        ...
        getJdbcTemplate().batchUpdate(sql, new BatchPreparedStatementSetter() {
            ...
            public void setValues(PreparedStatement ps, int i) throws SQLException {
                AcademyAward AcademyAward = access.get(i);
                //ps.setString(1, AcademyAward.getIp_address());
                //ps.setString(2, AcademyAward.getCommentary());
            }
